        ...
        if( ($match = $this->canParse($url->host)) !== false){
            ...
            $result = null;
                ...
                if($url->path[0] == 'swf' || $url->path[0] == 'video' ){
                    ...
                    $result=$dailymotion->get('/videos',array('fields'=>array('id','title','thumbnail_120_url','duration'),'ids'=>array(end($url->path) ) ));
                }else{
                    $path=end($url->path);
                    $lastPath=explode('_', $path);

                    $result=$dailymotion->get('/videos',array('fields'=>array('id','title','thumbnail_120_url','duration'),'ids'=>array($lastPath[0]) ) );
